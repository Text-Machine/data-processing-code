...
Dataset-agnostic: works on output from any of the BL Microsoft,
...
def make_output_name(filepath: Path) -> str:
    """
    Auto-detects an existing step tag anywhere in the filename (matching
    both "_step_4" -- BL Microsoft/HMD/LWM's underscore-before-digit style
    -- and "_step2" -- the TCP pipeline's no-underscore style) and
    increments it by one, appending the result at the end of the stem.

        bl_microsoft_step_4_slaves_spacy.jsonl -> bl_microsoft_slaves_spacy_step_5.jsonl
        lwm_machine.jsonl                      -> lwm_machine_step_5.jsonl   (no tag found -> default)
        hmd_slaves.jsonl                       -> hmd_slaves_step_5.jsonl    (no tag found -> default)
        evans_machine_step2.jsonl              -> evans_machine_step3.jsonl
        eebo_machine_step2.jsonl               -> eebo_machine_step3.jsonl

    If no step tag is found at all, falls back to appending "_step_5" (the
    original hardcoded default), preserving behavior for any input that
    predates this auto-detection.
    """

    stem = filepath.stem

    match = re.search(r"_step_?(\d+)", stem)

    if match:
        current_step = int(match.group(1))
        # Preserve whichever separator style was actually used, rather
        # than assuming one convention -- "_step_" vs "_step".
        separator = "_step_" if match.group(0).startswith("_step_") else "_step"
        next_step = current_step + 1

        # Remove the matched tag from wherever it appears in the stem
        # (BL Microsoft embeds it mid-filename, not at the end) and
        # append the incremented tag at the end.
        stem = stem[:match.start()] + stem[match.end():]
        stem = f"{stem}{separator}{next_step}"
    else:
        stem = f"{stem}_step_5"
