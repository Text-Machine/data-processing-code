...
filter_data_blmicrosoft_spacy.py) SUFFIX="spacy" ;;
filter_data_blmicrosoft_regex.py) SUFFIX="regex" ;;
*)
  echo "ERROR: unrecognized SCRIPT '$SCRIPT_BASENAME'" >&2
  exit 1
  ;;
esac
echo "Using script: $SCRIPT_BASENAME  ->  suffix: step_4_${SUFFIX}"
echo "Words this run: ${WORDS[*]}"
...
for word in "${WORDS[@]}"; do
  out_file="${OUTPUT_FILES[$word]}"

  if [[ -s "$out_file" ]]; then
    echo "Skipping '$word' — output already exists: $out_file"
    continue
  fi
