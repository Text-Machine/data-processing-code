...
Sentence splitting is done with spaCy's rule-based sentencizer instead of a
regex, to better handle abbreviations, OCR artefacts, etc.
"""
...
# Fallback defaults — overridden by CLI args in main(), and also accepted as
# explicit parameters everywhere else so nothing silently falls back to a
# stale global when called from the launcher.
...
# ---------------------------------------------------------------------------
# spaCy sentencizer (loaded once per process)
# ---------------------------------------------------------------------------
...
def get_tar_path(path_to_json: str, data_root: Path) -> Path | None:
...
def iter_page_matches(
    ...
    """
    tar_path = get_tar_path(path_to_json, data_root)
    if tar_path is None:
        return
            ...
            for record_id, meta_record in metadata.items():
                ...
                try:
                    for record in iter_page_matches(
                        path_to_json, meta_record, query_words, pattern, tar_cache, data_root
                    ):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-root",  type=Path, default=DATA_ROOT,
                        help="Root directory containing the tar.gz archives")
