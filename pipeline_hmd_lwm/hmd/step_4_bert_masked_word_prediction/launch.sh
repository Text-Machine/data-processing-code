#!/bin/bash
#SBATCH --job-name=bert_preds_man_DATASET
#SBATCH --output=logs/bert_preds_man_DATASET_%j.log
#SBATCH --error=logs/bert_preds_man_DATASET_%j.err
...
# Input/output point at the per-dataset step-5 output produced by
# filter_hmd_lwm.py, not the BL Microsoft "consolidated_metadata"
# directory — HMD and LWM each get their own dir so a "man" run against
# one dataset never collides with (or accidentally completeness-checks
# against) the other's output.
#
# NOTE: paths must include the underscore before the step number
# (output_hmd_step_5, not output_hmd_step5) to match what's actually on
# disk. If you'd rather use the script's own built-in hmd/lwm defaults
# instead of overriding here, just delete these two export lines.
export BERT_INPUT_DIR="/gpfs/projects/bsc100/textmachine-data/preprocessed_data/consolidated_metadata_hmd"
export BERT_OUTPUT_DIR="/gpfs/projects/bsc100/textmachine-data/preprocessed_data/consolidated_metadata_hmd"
...
python3 run_predictions_hmd.py
