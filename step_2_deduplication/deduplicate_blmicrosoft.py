...
**Phase 2 — Inference** (fully automated, no user interaction)::
    ...
    python deduplicate_blmicrosoft.py --input metadata.csv [--output path/to/output.csv]

The saved model is loaded and applied to every author's subset of rows.
The output is written to the path given by ``--output`` (or, if omitted, to
``<input_stem>_deduplicated.csv`` in the same directory as the input) with two
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Writes the result (with two new columns: 'book_id' and 'best_version') "
            "to the path given by --output, or to <input_stem>_deduplicated.csv in "
            "the same directory as the input if --output is omitted."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to <input_stem>_deduplicated.csv in the same directory "
            "as the input file if not specified."
        ),
    )
    parser.add_argument(
        "--sample-train",
